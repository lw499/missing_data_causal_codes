...
myfunc = function(m)
{
  ...
  RAfit = glm(RA~LO*LM1*LM2, family = binomial(link=logit), data=tmpdata[tmpdata$RL==1,])
  tmpdata$predra = predict(RAfit, newdata = tmpdata, type="response")
  
  RLfit = glm(RL~LO, family = binomial(link=logit), data=tmpdata)
  tmpdata$predrl = predict(RLfit, newdata = tmpdata, type="response")
  ...
  fity1 = glm2(Y ~ 1, offset = predo, weight = 1/(preda*predra*predrl), family = binomial(link=logit), data = mydat)
  mydat = tmpdata[tmpdata$RL==1,]; mydat$A=1;
  mydat$predo = predict(fity1o, newdata = mydat)
  ...
  fity2 = glm2(pred1 ~ 1, offset = predo, weight = 1/(predrl), family = quasibinomial(link=logit), data = mydat)
  mydat = tmpdata
  mydat$predo = predict(fity2o, newdata = mydat)
