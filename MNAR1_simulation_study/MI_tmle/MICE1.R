...
myfunc = function(m)
{
  ...
  for (i in 1:impnum)
  {
    ...
    fitA = glm(A~LO+LM1+LM2, data=tmpdataimp_complete, family = binomial(link=logit))
    ...
    fit_glm <- glm2(Y ~ LO+LM1+LM2 + LM1*LM2 + LM1*LO, data = tmpdataimp_complete[tmpdataimp_complete$A==1,], weights=1/preda,family=binomial())
