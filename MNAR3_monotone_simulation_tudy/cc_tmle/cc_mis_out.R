...
myfunc = function(m)
{
  ...
  fitA = glm(A~LO*LM1*LM2, data=tmpdata, family = binomial(link=logit))
  tmpdata$preda = predict(fitA, newdata = tmpdata, type="response")
  tmpdata$preda = ifelse(tmpdata$A==1,tmpdata$preda, 1-tmpdata$preda)
  
  fit_glm <- glm2(Y ~ LO+I(LM1*LM2), data = tmpdata[tmpdata$A==1,], weights=1/preda,family=binomial())
  tmpdat = tmpdata;
...
write.csv(test2,"cc_mis_out.csv")
