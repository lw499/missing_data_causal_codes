...
datagen <- function(N)
{
  ...
  Astar = rbinom(N, 1, plogis(-1+LM1star-2*LM2star+2*LO)); 
  Y = rbinom(N, 1, plogis(-1 + Astar - LM1star - LM2star -2*LM1star*LM2star + LO - 2*LM1star*LO))
  R = rbinom(N, 1, plogis(2-2*LO)) #missing covariate indicator
  ...
  temp_data = data.frame(cbind(ids, LO, LM1, LM2 ,R, A,Y, Astar))
  return(temp_data)
}
